...
export default function Edit({ mustVerifyEmail, status, employees = [] }) {
    ...
    const startEdit = (emp) => {
        ...
        editForm.setData({
            ...
            actions: emp.permissions?.actions ?? {
                create: false,
                update: false,
                delete: false,
                impersonate: false,
    ...
    const actionLabels = {
        ...
        delete: 'Удаление',
        impersonate: 'Вход в кабинет вебмастера',
        impersonate_employee: 'Вход в кабинет сотрудника',
    };
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Личный кабинет</h2>}
        >
                    ...
                    {isAdmin && (
                        <div className="bg-white p-4 shadow sm:rounded-lg sm:p-8">
                            <h3 className="text-lg font-semibold text-gray-800">
                                Сотрудники партнёрской программы
                            </h3>
                            {flash?.success && (
                            ...
                            <form onSubmit={submitInvite} className="mt-3 grid gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="mt-2 flex flex-wrap gap-4 text-sm">
                                        ...
                                        <label className="inline-flex items-center gap-2">
                                            ...
                                            />
                                            {actionLabels.impersonate_employee}
                                        </label>
                                    </div>
                                </div>
                                <button
                                ...
                                {inviteForm.errors.email && <div className="text-xs text-red-600">{inviteForm.errors.email}</div>}
                            ...
                            <div className="mt-6 space-y-2">
                                {employees.length === 0 && (
                                    <div className="text-sm text-gray-500">Пока нет сотрудников</div>
                                )}
                                {employees.map((emp) => (
                                        ...
                                        <div className="flex items-start justify-between">
                                            <div>
                                                ...
                                                <div className="text-gray-500">
                                                    Разделы:{' '}
                                                    {(emp.permissions?.sections || [])
                                                        .map((s) => sectionLabels[s] ?? s)
                                                        .join(', ') || '—'}
                                                </div>
                                                <div className="text-gray-500">
                                                    Действия:{' '}
                                                    {Object.entries(emp.permissions?.actions || {})
                                                        .filter(([, v]) => v)
                                                        .map(([k]) => actionLabels[k] ?? k)
                                                        .join(', ') || '—'}
                                                </div>
                                            </div>
